...
import gachaIntegration from '../../services/gachaIntegration.js';
...
export default {
    data: new SlashCommandBuilder()
        ...
        .addSubcommand(subcommand =>
            subcommand
                .setName('populate-trophies')
                .setDescription('Retroactively award trophies for existing achievements')
                .addStringOption(option =>
                    option.setName('username')
                        .setDescription('Specific user (optional - leave blank for all users)')
                        .setRequired(false))),
        ...
        try {
            switch (subcommand) {
                ...
                case 'import-preset':
                    ...
                    break;
                case 'populate-trophies':
                    await this.handlePopulateTrophies(interaction);
                    break;
            }
        } catch (error) {
    ...
    async handlePopulateTrophies(interaction) {
        ...
        if (username) {
            ...
            const user = await User.findOne({ 
                raUsername: { $regex: new RegExp(`^${username}// src/commands/admin/gacha-admin.js
import { 
    SlashCommandBuilder, 
    EmbedBuilder, 
    AttachmentBuilder 
...
, 'i') }
        ...
        let totalTrophiesAdded = 0;
        let usersUpdated = 0;
        
        for (const user of users) {
            const trophiesAdded = await gachaIntegration.populateExistingTrophies(user);
            if (trophiesAdded > 0) {
                totalTrophiesAdded += trophiesAdded;
                usersUpdated++;
            }
        }
        
        await interaction.editReply({
            content: `✅ Trophy population complete!\n` +
                    `• **Users Updated:** ${usersUpdated}\n` +
                    `• **Total Trophies Added:** ${totalTrophiesAdded}\n` +
