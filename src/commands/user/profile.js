...
export default {
    ...
    createProfileButtons(user) {
        ...
        return new ActionRowBuilder().addComponents(trophyButton);
    },
    ...
    async handleButtonInteraction(interaction) {
        ...
        try {
            // Parse the customId - should be profile_trophy_case_USERNAME
            if (!interaction.customId.includes('trophy_case')) {
                console.error('Unknown profile button action:', interaction.customId);
                return;
            }

            const username = interaction.customId.replace('profile_trophy_case_', '');

            // Find the user
            const user = await User.findOne({ 
                raUsername: { $regex: new RegExp(`^${username}// src/commands/user/profile.js
import { 
    SlashCommandBuilder, 
    EmbedBuilder,
    ActionRowBuilder,
