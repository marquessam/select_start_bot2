...
class RetroAchievementsAPI {
  constructor(username, apiKey) {
    ...
    console.log('RetroAchievementsAPI initialized with username:', this.username);
  }
    ...
    try {
      ...
      if (error.response && error.response.status === 429) {
        if (retryCount < this.maxRetries) {
          console.warn(`Rate limited on ${endpoint}, attempt ${retryCount + 1}/${this.maxRetries}. Retrying in ${this.retryDelay}ms...`);
